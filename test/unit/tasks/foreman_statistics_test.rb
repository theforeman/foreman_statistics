...
class ForemanStatisticsTest < ActiveSupport::TestCase
  ...
  test 'foreman_statistics:trends:reduce does not iterfere between two trend ids' do
    trend1 = FactoryBot.build(:foreman_statistics_trend_os)
    trend2 = FactoryBot.build(:foreman_statistics_trend_os)
    create_trend_line(trend1, [1, 1, 1, 1, 1, 1])
    create_trend_line(trend2, [2, 2, 2, 2, 2, 2])

    Rake.application.invoke_task 'foreman_statistics:trends:reduce'

    trend_10 = ForemanStatistics::TrendCounter.where(trend_id: trend1.id)
    trend_20 = ForemanStatistics::TrendCounter.where(trend_id: trend2.id)
